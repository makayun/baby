import { AbstractMesh } from "@babylonjs/core";
import * as GUI from "@babylonjs/gui";
export declare function createSimpleButton(text: string): GUI.Button;
export declare function createImageButton(text: string, imageUrl: string): GUI.Button;
export declare function createTextBlock(in_text: string, in_color: string, in_fontSize: number): GUI.TextBlock;
export declare function createInputText(in_text?: string): GUI.InputText;
export declare function createSlider(in_min: number, in_max: number): GUI.Slider;
export declare function createPicker(): GUI.ColorPicker;
export declare function addLabelToMesh(mesh: AbstractMesh): void;
//# sourceMappingURL=gui.d.ts.map